                      ...
                      <table class="table" id="example">
                        ...
                        <?php
                            ...
                            $tampil = mysqli_query($koneksi, "
                                SELECT p.*, pl.nama_221042 as nama_pelanggan, m.nomor_meja_221042, pb.status_221042 as status_pembayaran
                                ...
                                ORDER BY p.tanggal_pesanan_221042 DESC
                            ");
                            while($data = mysqli_fetch_array($tampil)):
                        ?>
                          <tr>
                            <td><?= $no++ ?></td>
                            <td><?= $data['kode_pesanan_221042'] ?></td>
                            <td><?= $data['nama_pelanggan'] ?? 'Tidak ada data' ?></td>
                            <td>
                              <?php if($data['nomor_meja_221042']): ?>
                                Meja <?= $data['nomor_meja_221042'] ?>
                              <?php else: ?>
                                <span class="text-muted">Tidak ada meja</span>
                              <?php endif; ?>
                            </td>
                            <td><?= date('d/m/Y H:i', strtotime($data['tanggal_pesanan_221042'])) ?></td>
                            <td><?= 'Rp ' . number_format($data['total_221042'], 0, ',', '.') ?></td>
                            <td>
                              <?php if(!empty($data['catatan_221042'])): ?>
                                <?= substr($data['catatan_221042'], 0, 50) ?>
                                <?= strlen($data['catatan_221042']) > 50 ? '...' : '' ?>
                              <?php else: ?>
                            ...
                            <td>
                              <?php 
                                $status_pembayaran = $data['status_pembayaran'] ?? 'Menunggu Pembayaran';
                            ...
                            <td>
                                <a class="btn btn-info btn-sm" href="detailpesanan.php?kode=<?= $data['kode_pesanan_221042']?>">Detail</a>
                                <a class="btn btn-warning btn-sm" href="editpesanan.php?hal=edit&kode=<?= $data['kode_pesanan_221042']?>">Edit</a>
                            </td>
                          </tr>
                          <?php
                            endwhile; 
